...
float scaled_colors[width][height][3];
...
void* draw(void* arg)
{
    ...
    for( int px=threadNum; px<width; px+=nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            scaled_colors[px][py][0] = scaled_color[0];
            scaled_colors[px][py][1] = scaled_color[1];
            scaled_colors[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    int *rows = (int*)malloc(nthreads*sizeof(int));
    if (rows == NULL)
    {
        fprintf(stderr, "Error allocating memory.\n");
        exit(-1);
    ...
    {
        rows[i] = i;
        int ret = pthread_create(threads+i, NULL, draw, &rows[i]);
        if (ret)
        {
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        {
            printf("%.0f %.0f %.0f\n",
                   scaled_colors[i][j][0], scaled_colors[i][j][1], scaled_colors[i][j][2]);
        }
        printf("\n");
    }
